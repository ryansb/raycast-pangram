...
async function pollTask(
	...
	for (let i = 0; i < POLL_MAX_ATTEMPTS; i++) {
		...
		if (data.stage === "STAGE_FAILED")
			throw new Error("Pangram analysis failed");
	}
	...
	useEffect(() => {
		async function loadText() {
			try {
				...
				if (!t) {
					setErrorMessage(
						"Select or copy some text before running this command.",
					);
					setPhase("error");
					return;
				}
